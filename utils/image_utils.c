...
// 文件说明：
// 实现图像加载、保存、颜色转换以及 RGA/CPU 预处理辅助逻辑。
...
static int g_logged_force_cpu_env = 0;

static int is_true_env_value(const char *value) {
...
static int should_force_cpu_convert(const image_buffer_t *src_img,
                                    const image_buffer_t *dst_img) {
    const char *disable_rga_env = getenv("RKNN_DEMO_DISABLE_RGA");
    if (is_true_env_value(disable_rga_env)) {
        if (!g_logged_force_cpu_env) {
            printf("image_utils: RKNN_DEMO_DISABLE_RGA=%s, force CPU image conversion\n",
                   disable_rga_env);
            g_logged_force_cpu_env = 1;
        }
        return 1;
    }
    ...
    for (int dst_y = dst_box_y; dst_y < dst_box_y + dst_box_height; dst_y++) {
        for (int dst_x = dst_box_x; dst_x < dst_box_x + dst_box_width; dst_x++) {
            ...
            for (int c = 0; c < channel; c++) {
                ...
                /*
                 * 当输入输出一个是 RGB、另一个是 BGR 时，R/B 通道需要交换。
                 * 这里把“交换通道”融进缩放写回阶段，避免额外再跑一次颜色转换。
                 */
                int dst_channel = c;
                if (swap_rb && channel == 3 && (c == 0 || c == 2)) {
                    dst_channel = 2 - c;
                }
                dst[(dst_y * dst_width  + dst_x) * channel + dst_channel] = pixel;
            }
        }
    }
...
static int crop_and_scale_image_yuv420sp(unsigned char *src, int src_width, int src_height,
    ...
    crop_and_scale_image_c(1, src_y, src_width, src_height, crop_x, crop_y, crop_width, crop_height,
        dst_y, dst_width, dst_height, dst_box_x, dst_box_y, dst_box_width, dst_box_height, 0);
    
    crop_and_scale_image_c(2, src_uv, src_width / 2, src_height / 2, crop_x / 2, crop_y / 2, crop_width / 2, crop_height / 2,
        dst_uv, dst_width / 2, dst_height / 2, dst_box_x, dst_box_y, dst_box_width, dst_box_height, 0);
...
static int convert_image_cpu(image_buffer_t *src, image_buffer_t *dst, image_rect_t *src_box, image_rect_t *dst_box, char color) {
    ...
    /*
     * CPU 回退路径以前只允许“输入输出格式完全相同”。
     * 现在增加了 RGB888 <-> BGR888 的直通支持。
     */
    if (src->format != dst->format) {
        if (!((src->format == IMAGE_FORMAT_RGB888 || src->format == IMAGE_FORMAT_BGR888) &&
              (dst->format == IMAGE_FORMAT_RGB888 || dst->format == IMAGE_FORMAT_BGR888))) {
            return -1;
        }
        swap_rb = 1;
    }
...
static int get_rga_fmt(image_format_t fmt) {
    ...
    /*
     * 让 RGA 直接识别 BGR888，这样主流程就不需要先用 cv::cvtColor 把 BGR 转成 RGB。
     */
...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        /*
         * letterbox 需要先把整张目标图填成背景色，再把缩放后的图像贴到中间区域。
         *
         * 在部分 RK3588 板端环境里，RGA 对 importbuffer_virtualaddr() + RGB888/BGR888
         * 的 color fill 支持并不稳定，容易报：
         *   RGA_COLORFILL fail: Invalid argument
         *
         * 对当前推理输入路径来说，dst 通常就是一块普通 CPU 虚拟内存，
         * 这里直接用 memset 更稳定，也足够快。
         *
         * 只有当目标端不是可直接访问的虚拟内存时，才尝试走 RGA 的 imfill。
         */
        if (dst != NULL) {
...
#else
    if (should_force_cpu_convert(src_img, dst_img)) {
        ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color);
    } else {
